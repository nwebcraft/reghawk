...
システムは、情報源ごとに以下の関心領域を定義しなければならない（SHALL）。

| 情報源 | 関心領域キーワード | 判定方針 |
|--------|-------------------|----------|
| 金融庁 | 暗号資産、仮想通貨、暗号資産交換業、ブロックチェーン、Web3 | キーワード該当時のみ |
| 経済産業省 | 補助金、助成金、支援金、給付金、事業支援制度 | キーワード該当時のみ |
| 厚生労働省 | 社会保険、健康保険、厚生年金、雇用保険、労災保険、社会保障 | キーワード該当時のみ |
| デジタル庁 | DX、デジタルトランスフォーメーション、マイナンバー、デジタル化推進 | キーワード該当時のみ |
| 総務省 | （全般） | 常に該当 |
| e-Gov | パブリックコメント | 常に該当 |

#### Scenario: Define interest domains per source
- **WHEN** システムが初期化される
- **THEN** 各情報源に対応する関心領域キーワードが設定される
- **AND** 総務省とe-Govは全件該当として設定される
